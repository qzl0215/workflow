...
3. 表面请求与真实用户结果之间有方向缺口时，AI主动进入需求澄清，不依赖用户说出“澄清需求”；承重决策进 Grill 深度，用户选择、填写或委托，契约内细节由 AI 代选展示；标准/项目须确认。
...
|---|---|---|---|---|
| 需求澄清 | 请求、事实、约束、真源 | 目标、验收、未知与授权边界 | 价值门 A、调研充分性硬门、需求成熟度硬门、契约稳定门通过；承重决策已处置，AI 代选细节已展示；标准/项目契约确认 | 调查、追问或 blocker |
...
**决策价值过滤器**用于所有用户可见内容：优先表达用户结果、实际效果、真实代价、关键风险和必要行动；仅展示会改变决定、行动、风险认知或对结论的信心的细节。承重决策由用户选择、填写或明确委托；契约内安全可逆项由 AI 代选并随契约展示。业务价值以证据为准；失败事实、关键风险、授权边界和必要证据保持透明。除等待用户回复外，H0/H1 只在开始、真实 blocker、完成时简短播报；H2/H3 或跨上下文时，状态变化后下一条消息给完整快照再继续。
...
| 取舍待定 | 承重选择 | 给推荐、选项与影响；用户决定或委托，细节由 AI 代选展示 | 决定或明确委托 |
