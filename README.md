...
只在开始、环节实质变化、真实阻断和交回控制权时更新，状态没变不重复：

> 进度｜■■◆□ · 任务执行<br>
> 技能｜[任务编排](references/orchestrate.md) · [任务执行](references/execute.md)<br>
> 成果｜✓ 结果契约 · ✓ 计划 P01–P04 · ● P02 / T03 · ○ 结果验真<br>
> 路径｜P01 → ● P02 / T03 → P04

- `进度` 只展示当前实际进入的核心或条件环节，不预先凑满格。
- `技能` 只列本轮真正读取的 reference。
- `成果` 只列已接受结果、当前责任和下一验证点。
- `路径` 仅在多任务依赖存在时展示最短活动路径，不展开整张内部队列。

需要用户接棒时，正文按“结论 → 风险或待决策 → 建议下一步 → 最短回复建议”组织。模型自己能解决的事实和实现细节，不转化为用户交互。
...
```bash
git clone --depth 1 --branch 3.0.0 https://github.com/qzl0215/workflow.git workflow-3
cd workflow-3
