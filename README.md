...
让模型完成已授权的真实任务，用有效证据验收，并沿项目入口完成交付。

4.0 将默认交互改为：范围清楚就执行；重要取舍或权限缺口才询问。用户要求先看方案时仍先给方案。技能不规定固定确认轮次、演示、报告格式或完成后复盘；具体方法由模型选择。
...
直接说明要完成的任务、重要限制和验收要求。纯问答与只读审查直接回答；明确的小修改直接执行。方向难以判断时才做演示，多任务有依赖时才展开计划，宿主允许且有收益时才协作。

参考按缺口读取，不按阶段全量加载。项目初始化仅用于明确接入、迁移或实际结构性缺口；缺少 `.workflow/project.json` 不影响普通工作。上下文压缩后从当前事实恢复，不自动启动治理。

现有页面优先复用真实源码与已有预览；不为局部调整默认新增验收路由。汇报说明实际变化、证据和未覆盖项，不能将包校验通过说成已经实际可用。
...
[行为评测说明](evals/README.md) 区分结构检查、决策探针和实际任务验收。两种模型使用同一精简技能；没有反复出现的行为证据，不增加模型专用规则。4.0 的实际样本、失败和限制见 [评测记录](evals/4.0.0-results.md)。
...
```bash
git clone --depth 1 --branch 4.0.0 https://github.com/qzl0215/workflow.git workflow-4
cd workflow-4
