...
长期项目优先复用 `AGENTS.md`、README、TRUTH、代码、配置和测试作为一个短小的项目知识导航，不把代码事实复制成另一份百科。AI 默认只读活动 Plan；完成 Plan 立即退出日常上下文，只有审计或具体历史缺口才显式追溯。

完成 Plan 不等于立刻删除。workflow 会先检查用户决定、方案理由、失败实验、外部验收、回滚证据和遗留风险是否已有 owner 与恢复位置；条件不全就保留为冷证据，只有检查通过且获得精确删除授权才物理删除。
...
| 完成 Plan 退出热区 | 先退出默认上下文，再经退休检查决定保留冷证据或授权删除 |
