...
4.1 在 4.0 的直接执行基础上强化主动引导：发现目标与方法冲突时给出有依据的修正建议；重要决定优先提供简短选项、推荐和取舍。能在上游解决的问题不转嫁给下游，收尾只补关键证据。用户要求先看方案时仍先给方案，不增加固定阶段、确认轮次、演示或汇报格式。
...
现有页面优先复用真实源码与已有预览；不为局部调整默认新增验收路由。汇报先说实际结果、必要缺口和下一行动；能自行推进就继续，需要用户时给出最小动作或有取舍的选择，目标完成就结束。不能将包校验通过说成已经实际可用。
...
[行为评测说明](evals/README.md) 区分结构检查、决策探针和实际任务验收。两种模型使用同一技能；没有反复出现的行为证据，不增加模型专用规则。实际样本、失败和限制见 [4.1 评测](evals/4.1.0-results.md) 与 [4.0 评测](evals/4.0.0-results.md)。
...
```bash
git clone --depth 1 --branch 4.1.0 https://github.com/qzl0215/workflow.git workflow-4
cd workflow-4
