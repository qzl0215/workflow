...
H0/H1 只在开始、真实 blocker、完成时简短播报；H2/H3、跨上下文或需要人接棒时，进度、实际使用的 reference、有效成果或当前 Plan/Task 发生变化会在下一条可见消息顶部给出一次最新快照，然后继续工作。状态没变就不重复。快照固定使用普通文本与 Markdown 链接，不探测宿主、不生成 DAG 视觉，也不展示 Ready 队列。有正式计划时只补一行最短活动路径，例如 `✓ P01 → ● P02 / T03 → ○ P03`。
