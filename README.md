...
| 核心环节 | 核心问题 | 退出依据 |
|---|---|---|
| 目标框定 | 究竟要改变谁的什么状态？ | 目标与验收清楚，会改变行动的边界按需补齐 |
| 结果规划 | 推荐什么方案，怎样把结果变成有责任和证据路径的工作？ | 用户确认或委托方案，每项验收都有实现责任和验真路径 |
...
workflow 先把用户的意见、设想或原则整理成候选目标。默认只需要两项：要产生什么真实变化，以及看到什么证据才算成功；范围、授权和风险只在确实会改变方案时补充。

首次状态变更前，模型展示目标、推荐方案、验收和交付路径。用户确认、明确委托或说“开动”后，方案进入执行；“开动”同时让本次确认在实施、验真、提交、合并、发布和发布后检查之间持续有效。新事实只有实质改变结果、风险、代价或授权时，才重新回到用户决定。
