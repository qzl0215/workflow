...
4. 在最终工作树生成一次完整 RC 回执；commit 或 merge 后做**内容等价检查**。command-scoped 内容、命令和环境相同就复用；无关目标前进不重跑，冲突消解、相关依赖或源码变化只失效受影响证据，形成新的完整输入才重建 full。
...
10. 面向用户固定报告三行：`代码完成｜`、`合并完成｜`、`线上生效｜`。三项都出现；用户汇报不得展示提交哈希、commit ID、digest 或内部快照名，精确技术标识只写审计真源。
