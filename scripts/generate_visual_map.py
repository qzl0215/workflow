...
ACTION_STORIES = {
    "需求澄清": {
        ...
        "solution": "先查清事实、分流未知，再把目标压成可验证的承诺。",
        "best_practice": "事实能查就查；只把会改变方向的取舍交给用户。",
        "question": "为什么做，怎样才算成？",
        "ai": "查事实、分未知、收边界。",
        "user": "确认价值、优先级与授权。",
        "outcome": "目标、成功信号与非范围。",
        ...
        "deep": ("四路未知：事实可查就查，取舍待定再问，假设待验就试，外部待解则等授权。", "关键追问：只问会改变方向的问题。"),
    },
    "选定方案": {
        "promise": "先看见终局，再选择路径。",
    ...
    "执行任务": {
        "promise": "一次做成一个可验收结果。",
        "pain": "多任务同时开工、上下文越滚越大，失败后继续堆补丁。",
        "solution": "一次领取一个就绪任务，在限定文件域内产出并完成任务级自验。",
        "best_practice": "从第一个错误状态修根因；交接只携带当前任务需要的上下文。",
        "question": "现在最该做成哪一项？",
        "ai": "领取就绪任务，产出并自验。",
        "user": "只确认范围或方向变化。",
        ...
        "case": "只做当前就绪任务；重复追问先追到语义缺口，不继续堆补丁。",
        "deep": ("失败：找到第一个错误状态，修根因后再继续。", "交接：只带当前任务需要的事实、边界与证据。"),
    },
    "验收交付": {
        "promise": "先证明做成，再按授权交付。",
        "pain": "“代码写完”被当作“业务完成”，验证和外部授权混成一步。",
        "solution": "先验真，再按项目发布真源完成授权内集成发布。",
        "best_practice": "按项目文档提交合并，集成后重验并核对发布。",
        "question": "凭什么完成，能交到哪？",
        "ai": "验真、读发布真源、完成集成。",
        "user": "验收并授权交付目标。",
        ...
        "deep": ("第一道门：证据证明结果。", "第二道门：授权决定交付层级。", "第三道门：项目发布真源驱动集成发布与发布后核验。", "并行成果：各自验完，再串行合入；冲突保留双边意图，并重跑双方验证。"),
    },
    "提炼经验": {
    ...
    "回灌改进": {
        "promise": "证据够，才把经验变成规则。",
        "pain": "一次偶发经验被永久写入规则，真源越堆越多、彼此冲突。",
        "solution": "满足复用门后找到唯一 owner，用失败验收驱动最小规则更新。",
        "best_practice": "两次独立复现，或一次高影响且可复现，才晋升跨项目机制。",
        "question": "什么值得永久改变？",
        "ai": "找唯一真源，先写失败验收。",
        "user": "授权跨项目或外部变更。",
        "outcome": "最小真源更新，或保持不变。",
        "evidence": "复用门与新验证都通过。",
        "next": "下一次需求澄清",
        "fallback": "继续积累项目证据",
        ...
        "harnesses": ("coordinate-agents.md",),
        "documents": ("目标真源",),
        "case": "先用失败验收证明协议缺口，再改唯一来源并同步所有读写链。",
        "deep": ("两次独立复现，或一次严重且可复现，才升级长期机制。", "能补现有来源就不新建文档。"),
    },
}
...
- 入口：plans/260720-需求助手/""",
    "目标真源": """## 目标真源｜references/challenge-decisions.md
- 新规则：关键问题尽量一次合并询问。
- 证据：三个项目因分轮追问重复返工。
- 边界：事实类问题仍由 AI 自行调查。
- 验收：新案例一轮闭合目标、取舍与授权。
...
def build() -> tuple[str, str]:
    ...
    if current_release is None or any(name not in current_release.group(0) for name in ACTION_STORIES):
