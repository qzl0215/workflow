...
状态只允许 `promoted / already_owned / intentionally_ephemeral / unresolved`；规则由“提炼经验”owner 持有。

| 信息类别 | 状态 | 长期 owner / 证据位置 | 恢复位置 / 保留期限 |
|---|---|---|---|
| 用户决定、范围与授权 | | | |
| 采用与拒绝方案，以及失效条件 | | | |
| 稳定事实与规范不变量 | | | |
...
- 处置：活动 / 已退出默认上下文；保留冷证据 / 等待精确删除授权 / 已授权删除。
- 任一项 `unresolved`、无 owner、无恢复位置或无精确删除授权时，不得物理删除。
