...
- 任何线程发生上下文压缩时，强制进入经验复盘门，审计 workflow 或项目级 `harness`；压缩只触发检查，不预设存在缺陷，完成检查后仍可 `no-op`。
- 专项检查覆盖信息重复冗余、唯一真源精炼度、导航与恢复入口，以及缺失最小恢复上下文造成的大段重载。
- 对实际业务流转逐项执行奥卡姆删除测试：删除或合并后若不降低目标结果、风险覆盖、可恢复性和证据强度，就移除与目标关联较弱的汇报、状态搬运、代理性产物、重复验证或等待。
- 修改现有系统时默认保护结果契约之外的可观察能力，优先以边界与原实现复用证明保真；共享边界变化才扩大相邻回归。
