...
- 把方案、研究结论、演示、局部实现、测试和部署记录明确为推进输入；下一动作仍在授权内时继续执行，不把进度更新或中间产物变成新的确认门。
- 真实交回控制权时说明当前处境、原目标是否完成、停止原因、下一责任者及恢复路径；用户可以通过最短回复接受推荐、选择备选或委托决定。
- 体验探索把演示绑定到要支持的决定，区分方向确认与成品验收；规划、验真和长任务恢复同步保留用户专属操作、完成信号及后续动作。
- 新增演示完成、用户专属操作、权限解锁、研究结论和运行中进度等行为场景，以及不修改项目的演示交接任务夹具；继续区分动作筛查、语义复核与真实多轮会话证据。
- 不恢复固定阶段画面、强制演示、逐步批准或完成后的虚构待办；运行时接口、安装与发布机制及项目兼容代际保持不变。

## [4.1.0] - 2026-09-22

### 主动共创与轻量推进

- 主动识别用户目的与请求手段的冲突、关键遗漏和错误前提，提供有依据的修正建议；保留重要业务取舍与实施授权边界，不把主动性变成擅自改目标。
- 在最早能低成本、可靠消除成因的位置解决问题；规划与执行取得必要证据，收尾只补关键缺口，不默认全量回放或另建验收系统。
- 汇报简短直观，说明当前结果和下一行动；需要用户决定时推荐在前、给少量真实选项与关键取舍，能自行推进就继续，完成后不制造新审批。
- 明确项目事实、当前任务与通用方法的归属；记忆按原责任位置更新、保留新旧决定关系，不复制知识库，不把建议或计划写成政策与已上线事实。
- 重复问题优先处理目标、共享成因和已有自动检查，不只叠加下游补丁及提醒。运行路径、安装与合并工具、项目兼容代际不变。
- 扩展既有行为评测的目标冲突和项目记忆场景，支持先测旧版再测候选、固定参考快照；报告区分机械筛查与语义验收，不宣称样例代表普遍能力提升。

## [4.0.0] - 2026-09-20

### Changed

- 明确任务直接执行，取消默认需求/方案双确认、全任务演示、90% 自报门槛、固定阶段画面和空复盘；用户要求先看方案时仍等待确认。
- 保留目标、权限、用户现场、有效证据和连续交付边界；参考只在有缺口时加载，初始化与复盘退出默认路径。
- 精简长任务模板，确认凭证与身份摘要不再是普通任务前置；现有任务提取接口与兼容代际不变。
- 保留安装、Git 集成、失败恢复与 3.9 开发基准同步；发布版本校验支持后续主版本，继续核对候选精确版本。
- 用行为场景和可复查模型探针替代固定措辞测试；生成说明页直接消费技能内容，避免复制政策。

## [3.9.0] - 2026-09-20

### 交付后同步本机开发基准

- 远端目标确认合入后，复用已有提交对象与仓库互斥，将明确配置的本机开发基准安全快进；覆盖首次成功、传输重试、已合入与发布恢复路径，不追加 fetch 或重复验证。
- 基准由项目本机配置或显式参数指定；只操作同仓库已登记、未锁定且检出目标分支的开发基准，未使用 Git 或未配置时跳过，不猜任务目录或生产目录。
- 脏现场、分叉、进行中的 Git 操作与未完成集成均保留并报告；禁止强制覆盖、自动 stash 或 rebase，忽略文件也不被快进覆盖。远端交付和本机同步分别报告，同步受阻可恢复重试。
- 其他机器或绕过 Workflow 的远端更新，在下次任务启动时单次检查并安全同步，不增加轮询、常驻服务或即时感知承诺。
